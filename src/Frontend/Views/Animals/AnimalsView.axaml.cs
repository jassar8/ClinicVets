...
namespace ClinicVets.Desktop.Views.Animals;
...
public partial class AnimalsView : UserControl
{
    ...
    private async void AddAnimal_Click(object? sender, RoutedEventArgs e)
    {
        ...
        var species = SpeciesDropdown.SelectedItem is ComboBoxItem item ? item.Content?.ToString() ?? "???" : "???";
    ...
    private void UpdateAnimalActionMode()
    {
        if (SaveAnimalButton is not null)
            SaveAnimalButton.Content = _isAdding ? "???? ???" : "???? ???";
    }
    ...
    private void RefreshAnimalsList()
    {
        ...
        if (AnimalResultsText is not null)
            AnimalResultsText.Text = $"???? {filtered.Count} ???? {_items.Count} ???? ????";
        ...
        var card = new Button
        {
            ...
            Content = new StackPanel
            {
                ...
                Children =
                {
                    ...
                    new TextBlock { Text = $"?????: {row.Owner.FullName}", FontSize = 12, TextWrapping = TextWrapping.Wrap, TextAlignment = TextAlignment.Center }
                }
            }
        };
        card.Click += (_, _) =>
