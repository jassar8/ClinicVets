        ...
        lock (_sync)
        {
            if (File.Exists(_filePath))
            {
                try
                {
                    ...
                    _employees = JsonSerializer.Deserialize<List<Employee>>(json, JsonOptions) ?? [];
                }
                catch
                {
                    // Corrupt file: keep empty list and re-bootstrap admin only (do not wipe registered users).
                    _employees = [];
                }
            }
            else
            {
                // First install: seed demo staff alongside bootstrap admin.
                _employees = CreateDefaultEmployees();
            }
