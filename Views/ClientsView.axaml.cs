...
namespace ClinicVetsAvalonia.Views
{
    public partial class ClientsView : UserControl
    {
        ...
        private void SearchClient_Click(object? sender, RoutedEventArgs e)
        {
            ...
            {
                searchValue = searchMode switch
                {
                    "תעודת זהות" => IdNumberInput.Text?.Trim() ?? "",
                    "טלפון" => PhoneInput.Text?.Trim() ?? "",
                    "שם" => FullNameInput.Text?.Trim() ?? "",
                    "אימייל" => EmailInput.Text?.Trim() ?? "",
                    _ => ""
                };
            }
            ...
            if (string.IsNullOrWhiteSpace(searchValue))
            {
                UIHelper.ShowMessage(this, "בחר סוג חיפוש והכנס ערך לחיפוש");
                return;
            }

            var client = AppData.Clients.FirstOrDefault(c => MatchesClientSearch(c, searchMode, searchValue));
        ...
        private void ClearFields()
        {
            ...
            ClientSearchModeDropdown.SelectedIndex = 0;
        ...
        {
            if (ClientSearchModeDropdown.SelectedItem is ComboBoxItem item && item.Content != null)
                return item.Content.ToString() ?? "תעודת זהות";

            return "תעודת זהות";
        }

        private bool MatchesClientSearch(Client client, string searchMode, string searchValue)
        {
            return searchMode switch
            {
                "תעודת זהות" => client.IdNumber == searchValue,
                "טלפון" => client.Phone == searchValue,
                "שם" => client.FullName.Contains(searchValue, StringComparison.OrdinalIgnoreCase),
                "אימייל" => client.Email.Contains(searchValue, StringComparison.OrdinalIgnoreCase),
                _ => client.IdNumber == searchValue || client.Phone == searchValue
            };
        }
